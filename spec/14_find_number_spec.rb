...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:fixed_number) { double( value: 1) }
      subject(:matched_guess) { described_class.new( nil, nil, fixed_number, 1) }
      it 'is game over' do
        expect(matched_guess).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      let(:fixed_number) { double( value: 1)}
      subject(:invalid_guess) { described_class.new( nil, nil, fixed_number, 2)}
      it 'is not game over' do
        expect(invalid_guess).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:specific_game) { described_class.new(5, 8, number_range, 7)}
      it 'updates min to the same value as max' do
        specific_game.update_range
        expect(specific_game.min).to eq(8)
      end

      it 'does not update max' do
        specific_game.update_range
        expect(specific_game.max).to eq(8)
      end
    end
  end
end
